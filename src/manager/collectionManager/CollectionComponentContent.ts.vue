  ...
  <v-container class="py-0 px-0 collection-component-content">
    ...
    <v-sheet v-else-if="items.length > limit" class="ma-2 pl-2">
      <VcsButton @click="openCollectionComponentList">
        {{ $st('collectionManager.more') }}
      </VcsButton>
    </v-sheet>
    <v-sheet v-else-if="items.length === 0" class="ma-2 pl-2">
      {{ $st('collectionManager.empty') }}
    </v-sheet>
  </v-container>
</template>
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(_props, { emit }) {
      ...
      let destroyPaginationListeners: (() => void) | undefined;

      async function updatePagination(): Promise<void> {
        ...
        destroyPaginationListeners = (): void => {
          listeners.forEach((l) => {
            l?.();
          });
        };
      ...
      if (pagination.value) {
        updatePagination().catch((e: unknown) => {
