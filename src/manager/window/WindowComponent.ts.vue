...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(props, { emit }) {
      ...
      const isDynamic = computed(() => props.slotWindow.value !== 'static');
      const isDocked = computed(() => props.slotWindow.value !== 'detached');
      ...
      const isDockedLeft = computed(
        () =>
          props.slotWindow.value === 'static' ||
          props.slotWindow.value === 'dynamicLeft' ||
          props.slotWindow.value === 'dynamicChild',
      );
      const isDockedRight = computed(
        () => props.slotWindow.value === 'dynamicRight',
      );
      ...
      let startEvent: DragEvent | null = null;
      ...
      const dragEnd = (endEvent: DragEvent): void => {
        if (isDraggable.value) {
          const movement = {
            dx: endEvent.clientX - (startEvent?.clientX ?? 0),
            dy: endEvent.clientY - (startEvent?.clientY ?? 0),
          };
          emit('moved', movement);
          startEvent = null;
          isDraggable.value = false;
          (endEvent.target as HTMLElement).parentElement!.ondragover = null;
