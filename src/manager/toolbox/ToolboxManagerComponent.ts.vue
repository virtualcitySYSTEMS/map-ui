  ...
  >
    <v-toolbar-items class="w-100 px-4 gc-1">
      ...
      >
        ...
        />
        <ToolboxActionSelect
          v-else-if="isSelectComponent(group)"
          :group="group"
          @toggle="openGroup"
          @click="
            bringToTop;
        ...
        />
        <VcsToolButton
          v-else-if="isSingleComponent(group)"
...
<script lang="ts">
  ...
  import type { ToolboxComponent } from './toolboxManager.js';
  import {
    defaultToolboxName,
    getComponentsByOrder,
    ToolboxType,
  } from './toolboxManager.js';
  import ToolboxActionSelect from './SelectToolboxComponent.ts.vue';
  import ToolboxActionGroup from './GroupToolboxComponent.ts.vue';
  ...
  import {
    ...
    isSelectComponent,
    isGroupComponent,
  } from '../../callback/toggleToolbarButtonCallback.js';
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const groups = computed(() =>
        app.toolboxManager.componentIds
          .map((id) => app.toolboxManager.get(id))
          .filter((c) => !!c),
      );
      ...
      function filterFunc(c: ToolboxComponent): boolean {
        return (
          (isSingleComponent(c) ||
            (isSelectComponent(c) && c.action?.tools?.length > 1) ||
            (isGroupComponent(c) &&
              c.buttonManager?.componentIds?.length > 0)) &&
          ((xs.value && c[deviceSymbol]?.mobile === true) ||
            (sm.value && c[deviceSymbol]?.tablet === true) ||
            (mdAndUp.value && c[deviceSymbol]?.desktop === true))
        );
      }
      ...
      function handleToolboxButton(): void {
      ...
      const toolboxHeight = computed(() => fontSize.value * 3 + 1);
      ...
      return {
        ...
        open,
        bringToTop,
        openGroup(group: ToolboxComponent): void {
          open.value = !!group;
          if (open.value) {
            bringToTop();
          }
