<script setup lang="ts">
  ...
  const buttonComponents = computed(() =>
    app.navbarManager.getAllForDevice(sm.value ? 'tablet' : 'desktop'),
  );
  ...
  const toolbarHeight = computed(() => fontSize.value * 3 + 8 + 16);

  const mapActions = getActions(ButtonLocation.MAP);
  const contentActions = getActions(ButtonLocation.CONTENT);
  const toolActions = getActions(ButtonLocation.TOOL);
  const projectActions = getActions(ButtonLocation.PROJECT);
  const shareActions = getActions(ButtonLocation.SHARE);
  const menuActions = getActions(ButtonLocation.MENU);
</script>
