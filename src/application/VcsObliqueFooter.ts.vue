...
<script lang="ts">
  ...
  import type { ObliqueImage } from '@vcmap/core';
  import {
    ObliqueMap,
  ...
  const i18nViewDirection: Record<ObliqueViewDirection, string> = {
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      function getImage(): ObliqueImage | null {
      ...
      function getTranslatedImageInfo(
        image?: ObliqueImage,
      ): Record<string, unknown> | null {
        if (image) {
          const imageInfo = Object.fromEntries(Object.entries(image)) as Record<
            string,
            unknown
          >;
          imageInfo.viewDirection = app.vueI18n.t(
      ...
      const currentImage = shallowRef<ObliqueImage | null>(getImage());
