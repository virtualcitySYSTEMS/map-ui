    ...
    <v-card>
      ...
      <div class="vcs-splash-screen-checkboxes px-8">
        ...
        <VcsCheckbox v-if="options.enableDontShowAgain" v-model="dontShowAgain">
          <template #label>
            <div class="pl-2">
              {{ $st('components.splashScreen.dontShowAgain') }}
            </div>
          </template>
        </VcsCheckbox>
      </div>
...
<script lang="ts">
  ...
  import type { SplashScreen } from '../uiConfig.js';

  /**
   * @returns The hash of the SplashScreen config.
   */
  export function getSplashScreenHash(app: VcsUiApp): string {
    const config = app.uiConfig.getByKey('splashScreen');
    const string = JSON.stringify(
      Object.entries(config?.value ?? {}).sort((a, b) =>
   ...
   * @returns true if the SplashScreenConfigHash of the local storage is different from the current SplashScreenConfigHash
   */
  export function shouldShowSplashSceen(app: VcsUiApp): boolean {
  ...
  export default defineComponent({
    ...
    props: {
      ...
      options: {
        type: Object as PropType<SplashScreen>,
        required: true,
      },
    },
    setup(props, { emit }) {
      ...
      function secondaryButtonClicked(): void {
