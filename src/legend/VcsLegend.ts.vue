      ...
      >
        <v-list class="pl-6 pb-2 pr-2">
          <div v-for="(item, idx) in entry.legend" :key="idx">
            <VcsButton
              v-if="isIframe(item) || isImage(item)"
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(props) {
       ...
       * @param id - iframe's html id
       */
      const setIframeHeight = (id: string): void => {
      ...
      const panels = ref<string[]>(handledEntries.slice());
