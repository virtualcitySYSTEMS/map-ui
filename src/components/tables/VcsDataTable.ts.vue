<template>
  <div class="vcs-data-table">
    ...
    <component
      ...
      :no-data-text="
        $attrs.noDataText ?? $st('components.vcsDataTable.noDataPlaceholder')
      "
      :no-results-text="
        $attrs.noResultsText ??
        $st('components.vcsDataTable.noResultsPlaceholder')
      "
      ...
      @update:options="
        (o: Record<string, unknown>) => $emit('update:items', { ...o, search })
      "
    >
        ...
        <tfoot>
          <tr class="v-data-table__tr">
            <td colspan="100" class="text-center pa-1">
              ...
              <span class="mx-2">{{
                $st('components.vcsDataTable.itemsPerPage')
              }}</span>
              <span class="mx-2">
                {{ itemsFrom }} - {{ itemsTo }}
                {{ $st('components.vcsDataTable.ofItems') }} {{ numberOfItems }}
              </span>
              <VcsButton
...
<script lang="ts">
  ...
  import VcsTreeviewSearchbar from '../lists/VcsTreeviewSearchbar.ts.vue';
  ...
  export default defineComponent({
    ...
    props: {
      baseComponent: {
        ...
        validator: (type: string) =>
          ['VDataTable', 'VDataTableServer'].includes(type),
      },
      headers: {
    ...
    setup(props, { attrs, emit, slots }) {
      ...
      const handleFilterInternal = (
        ...
        if (filter) {
          ...
          return Object.values(item).some((i) => {
            if (i) {
              const content = (i as string | number).toString();
              const translated = vm?.$st(content) ?? content;
              return (
      ...
      const updateItemsPerPage = (number: number): void => {
