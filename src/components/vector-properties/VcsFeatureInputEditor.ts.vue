    ...
    <v-row no-gutters>
      <v-col>
        <VcsLabel html-for="posX">
          {{ $st('components.editor.position.x') }}
        </VcsLabel>
      </v-col>
      <v-col>
        <VcsTextField
    ...
    <v-row no-gutters>
      <v-col>
        <VcsLabel html-for="posY">
          {{ $st('components.editor.position.y') }}
        </VcsLabel>
      </v-col>
      <v-col>
        <VcsTextField
    ...
    <v-row no-gutters v-if="is3D">
      <v-col>
        <VcsLabel html-for="posZ">
          {{ $st('components.editor.position.z') }}
        </VcsLabel>
      </v-col>
      <v-col>
        <VcsTextField
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup() {
      const { currentFeatures: features } = inject('manager') as EditorManager;

      if (
        features.value.length !== 1 ||
        features.value[0]?.getGeometry()?.getType() !== 'Point'
      ) {
        getLogger('VcsFeatureInputEditor').error(
      ...
      function transform(coordinates: number[]): number[] {
      ...
      function updateFromFeature(): void {
