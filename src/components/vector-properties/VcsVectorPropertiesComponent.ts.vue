<script setup lang="ts">
  ...
  import { vectorProperties } from './vectorPropertiesHelper.js';
  import { useComponentId } from '../composables.js';

  const scaleByDistanceDefault = [0, 1, 1, 1];
  const eyeOffsetDefault = [0, 0, 0];
  const dimensionsUpperCase = ['X', 'Y', 'Z'] as const;
  ...
  type Props = {
    modelValue?: VectorPropertiesOptions;
    valueDefault: VectorPropertiesOptions;
    properties?: readonly (keyof VectorPropertiesOptions)[];
  ...
  const props = withDefaults(defineProps<Props>(), {
    modelValue: () => VectorProperties.getDefaultOptions(),
  ...
  const emit = defineEmits<{
    (e: 'update:modelValue', value: VectorPropertiesOptions): void;
  }>();

  const localValue = useProxiedComplexModel(props, 'modelValue', emit);

  const visibleProperties = computed(() => new Set(props.properties));
  ...
  const availableAltitudeModes = computed(() =>
    [
      ...
      },
      {
        ...
        props: { disabled: props.is2DFeature },
      },
      {
  ...
  const storeyHeights = computed(() =>
    storeyHeightsKeys
      ...
      .reduce<
        Partial<
          Record<
            (typeof storeyHeightsKeys)[number],
            WritableComputedRef<number[]>
          >
        >
      >((acc, key) => {
        acc[key] = computed<number[]>({
          get() {
            if (Array.isArray(localValue.value[key])) {
              return localValue.value[key];
            } else if ((localValue.value[key] as number) > 0) {
              return [localValue.value[key] as number];
            }
            return [];
          },
          set(value) {
  ...
  const modelScale = computed(() =>
    dimensionsUpperCase
      ...
      .reduce<
        Partial<
          Record<
            (typeof dimensionsUpperCase)[number],
            WritableComputedRef<number | undefined>
          >
        >
      >((acc, dimension) => {
        const key: 'modelScaleX' | 'modelScaleY' | 'modelScaleZ' =
          `modelScale${dimension}`;
        acc[dimension] = computed<number | undefined>({
          get() {
            return localValue.value[key];
          },
          set(value) {
...
<template>
  <VcsFormSection
    ...
    :heading="
      ($attrs.heading as string) || 'components.vectorProperties.header'
    "
        ...
        <v-row v-if="visibleProperties.has('altitudeMode')" no-gutters>
          <v-col>
            <VcsLabel :html-for="`${cid}-vp-altitude-mode`"
              >{{ $st('components.vectorProperties.altitudeMode') }}
              <template #help>
                <ul>
                  <li v-for="mode in availableAltitudeModes" :key="mode.value">
                    {{ $st(mode.title) }}:
                    {{ $st(`components.vectorProperties.help.${mode.value}`) }}
                  </li>
                </ul>
              </template>
            </VcsLabel>
        ...
        <v-row v-if="visibleProperties.has('heightAboveGround')" no-gutters>
          <v-col>
            <VcsLabel :html-for="`${cid}-vp-height-above-ground`">{{
              $st('components.vectorProperties.heightAboveGround')
            }}</VcsLabel>
          </v-col>
          <v-col>
            <VcsTextField
        ...
        <v-row v-if="visibleProperties.has('groundLevel')" no-gutters>
          <v-col>
            <VcsLabel :html-for="`${cid}-vp-ground-level`">{{
              $st('components.vectorProperties.groundLevel')
            }}</VcsLabel>
          </v-col>
          <v-col>
            <VcsTextField
      ...
      >
        <v-col>
          <VcsLabel :html-for="`${cid}-vp-classification-type`">
            {{ $st('components.vectorProperties.classificationType') }}
          </VcsLabel>
        </v-col>
        <v-col>
          <VcsSelect
      ...
      <v-divider
        v-if="
          ...
          [
            ...
            visibleProperties.has(prop as keyof VectorPropertiesOptions),
          )
        "
      />
      <v-container class="px-1 py-0">
        ...
        >
          <v-col>
            <VcsLabel>{{
              $st('components.vectorProperties.aboveGround')
            }}</VcsLabel>
          </v-col>
        </v-row>
        <v-row v-if="visibleProperties.has('extrudedHeight')" no-gutters>
          <v-col>
            <VcsLabel
              ...
              >{{ $st('components.vectorProperties.extrudedHeight') }}</VcsLabel
            >
          </v-col>
          <v-col>
            <VcsTextField
              ...
              :rules="[
                (v: number) =>
                  v >= 0 || isNaN(v) || 'components.validation.notValid',
              ]"
            />
          </v-col>
        </v-row>
        <v-row v-if="visibleProperties.has('storeysAboveGround')" no-gutters>
          <v-col>
            <VcsLabel :html-for="`${cid}-vp-storeys-above`" class="px-4">
              {{ $st('components.vectorProperties.storeys') }}
            </VcsLabel>
          </v-col>
          <v-col>
            <VcsTextField
        ...
        >
          <v-col>
            <VcsLabel :html-for="`${cid}-vp-storey-heights-above`" class="px-4">
              {{ $st('components.vectorProperties.storeyHeights') }}
            </VcsLabel>
          </v-col>
          <v-col>
            <VcsChipArrayInput
        ...
        >
          <v-col>
            <VcsLabel>
              {{ $st('components.vectorProperties.belowGround') }}
            </VcsLabel>
          </v-col>
        </v-row>
        <v-row v-if="visibleProperties.has('skirt')" no-gutters>
          <v-col>
            ...
            >
              {{ $st('components.vectorProperties.skirt') }}
            </VcsLabel>
          </v-col>
          <v-col>
            <VcsTextField
        ...
        >
          <v-col>
            <VcsLabel :html-for="`${cid}-vp-storey-heights-below`" class="px-4">
              {{ $st('components.vectorProperties.storeyHeights') }}
            </VcsLabel>
          </v-col>
          <v-col>
      ...
      <v-container class="px-1 py-0">
        <v-row v-if="visibleProperties.has('modelUrl')" no-gutters>
          <v-col>
            <VcsLabel :html-for="`${cid}-vp-model-url`">
              {{ $st('components.vectorProperties.modelUrl') }}
            </VcsLabel>
          </v-col>
          <v-col>
            <VcsTextField
        ...
        <template v-if="Object.keys(modelScale).length">
          <v-row no-gutters>
            <v-col cols="6">
              <VcsLabel>{{
                $st('components.vectorProperties.modelScale')
              }}</VcsLabel>
            </v-col>
          </v-row>
          <v-row no-gutters class="pb-2">
        ...
        <v-row v-if="visibleProperties.has('modelHeading')" no-gutters>
          <v-col cols="6">
            <VcsLabel :html-for="`${cid}-vp-model-heading`">{{
              $st('components.vectorProperties.modelHeading')
            }}</VcsLabel>
          </v-col>
          <v-col>
            <VcsTextField
        ...
        <v-row v-if="visibleProperties.has('modelPitch')" no-gutters>
          <v-col cols="6">
            <VcsLabel :html-for="`${cid}-vp-model-pitch`">{{
              $st('components.vectorProperties.modelPitch')
            }}</VcsLabel>
          </v-col>
          <v-col>
            <VcsTextField
        ...
        <v-row v-if="visibleProperties.has('modelRoll')" no-gutters>
          <v-col cols="6">
            <VcsLabel :html-for="`${cid}-vp-model-roll`">{{
              $st('components.vectorProperties.modelRoll')
            }}</VcsLabel>
          </v-col>
          <v-col>
            <VcsTextField
        ...
        <v-row v-if="visibleProperties.has('modelAutoScale')" no-gutters>
          <v-col cols="6">
            <VcsLabel>{{
              $st('components.vectorProperties.modelAutoScale')
            }}</VcsLabel>
          </v-col>
          <v-col>
        ...
        <v-row v-if="visibleProperties.has('baseUrl')" no-gutters>
          <v-col>
            <VcsLabel :html-for="`${cid}-vp-base-url`">{{
              $st('components.vectorProperties.baseUrl')
            }}</VcsLabel>
          </v-col>
          <v-col>
            <VcsTextField
