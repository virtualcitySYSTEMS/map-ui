...
import type { Options } from 'ol/style/Icon.js';
...
type Icon = Record<string, { component: Component }>;
...
const nameCapitalized = (name: string): string => {
  return name.charAt(0).toUpperCase() + name.slice(1);
};

const prefixed = (map: Icon): Partial<IconAliases> => {
...
export function getColoredMapIcon(color: string): Options {
  return {
