...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(_, { attrs, slots }) {
      ...
      const thumbSize = computed(() => Math.floor((fontSize.value + 3) / 2));
      const trackSize = computed(() => Math.floor((fontSize.value + 3) / 6));
      const tickSize = computed(() => trackSize.value * 2);
      const paddingProvided = usePadding(attrs);
      const forwardSlots = getForwardSlots(slots, ['append']);
      const sliderRef = ref();
      return {
        hasAppendSlot: computed(() => !!slots.append),
        forwardSlots,
        sliderRef,
        thumbSize,
        tickSize,
