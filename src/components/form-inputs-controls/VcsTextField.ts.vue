<template>
  <v-text-field
    ...
    v-bind="$attrs"
    :prefix="forcePrefix ? undefined : ($attrs.prefix as string)"
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(props, { attrs, slots }) {
      ...
      const focused = ref<boolean>(!!attrs.autofocus);
      ...
      const visibleValue = computed({
        get() {
          if (
            ...
            const v = parseFloat(localValue.value as string);
        ...
        set(value) {
          ...
          if (props.type === 'number') {
            localValue.value = parseFloat(value as string);
          } else {
      ...
      const rules = computed((): ((v: string) => boolean)[] => {
        if (attrs.type === 'number' && Array.isArray(attrs.rules)) {
          return attrs.rules.map((rule) => {
            return (value: string) => rule(parseFloat(value));
          });
        } else {
          return attrs.rules as ((v: string) => boolean)[];
        }
      });
