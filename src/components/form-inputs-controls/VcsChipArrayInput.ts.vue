<template>
  ...
  >
    <VcsButton
      v-if="hasScrollbar && vcsChipArrayInput"
...
<script lang="ts">
  ...
  export default defineComponent({
    ...
    setup(props, { attrs, emit }) {
      ...
      const editValue = ref<string | number | undefined>(undefined);
      const newValue = ref<string | number | undefined>(undefined);
      const vcsChipArrayInput = ref<HTMLElement | null>(null);
      const hasScrollbar = ref(false);

      function updateHasScrollbar(): void {
        if (!props.column && vcsChipArrayInput.value) {
          hasScrollbar.value =
      ...
      function isValid(value: string | number): boolean {
        if (attrs.rules) {
          return !(attrs.rules as ((value: string | number) => boolean)[]).some(
            (r) => {
      ...
      function submitChange(e: Event): void {
        e.stopPropagation();
        e.preventDefault();
        const value = parseValue(editValue.value as string | number);
        if (!isValid(value)) {
          return;
        }
