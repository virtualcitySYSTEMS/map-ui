...
<script lang="ts">
  ...
  import VcsCheckbox from './VcsCheckbox.ts.vue';
  ...
  export default defineComponent({
    ...
    props: {
      ...
      modelValue: {
        type: [Array, String, Boolean, Number, Object] as PropType<
    ...
    setup(props, { attrs, slots, emit }) {
      ...
      function getTitle(item: string | boolean | number | object): string {
        if (props.itemText) {
          return props.itemText(item);
        } else {
          return (item as { title?: string })?.title ?? (item as string);
        }
      }
