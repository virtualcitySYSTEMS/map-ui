<script setup lang="ts">
  ...
  function isPositiveNumber(number: number): boolean | string {
    return (
      ...
      <v-container class="py-0 px-1">
        <template v-if="!isCesiumMap">
          ...
          <VcsCoordinate
            ...
            @blur="gotoViewpoint"
            @update:model-value="
              (v: Coordinate) => handleInput('groundPosition', v)
            "
            hide-z
          />
        </template>
        <template v-else>
          ...
          <VcsCoordinate
            ...
            @blur="gotoViewpoint"
            @update:model-value="
              (v: Coordinate) => handleInput('cameraPosition', v)
            "
          />
        </template>
        <template v-if="!isCesiumMap">
        ...
        <template v-if="isCesiumMap">
          <div v-for="key in ['heading', 'pitch', 'roll']" :key="key">
            <VcsLabeledSlider
              ...
              v-model="localValue[key as 'heading' | 'pitch' | 'roll']"
              @blur="gotoViewpoint"
              @update:model-value="gotoViewpoint"
            />
          </div>
